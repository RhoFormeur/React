  ...
  return (
    <MainLayout>
      ...
      <label>
        ...
        <button
          type="button"
          onClick={() => getData(yearInput.current.value, setData)}>Envoyer</button>
      </label>
      <table>
        <thead>
          <tr>
        ...
        {data && data.MRData.StandingsTable.StandingsLists[0].DriverStandings.length > 0 ? (
          <tbody>
            {data.MRData.StandingsTable.StandingsLists[0].DriverStandings.map((item, index) => {
              return (
                <tr key={index}>
                  <td>{item.position}</td>
